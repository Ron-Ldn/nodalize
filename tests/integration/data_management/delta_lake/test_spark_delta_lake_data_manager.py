...
from tests.common import build_spark_config, use_temp_folder
from tests.integration.data_management.base_test_data_manager import BaseTestDataManager


class TestIntegrationSparkDeltaLakeDataManager(TestCase, BaseTestDataManager):
    @property
    def calculator_type(self):
        return lambda app: SparkCalculator(
            app, spark_config=build_spark_config(use_delta_lake=True)
        )

    # Known failure: module 'numpy' has no attribute 'bool'
    # https://github.com/Ron-Ldn/nodalize/issues/3
    @property
    ...
    def data_manager(self):
        return DeltaLakeDataManager(self.temp_directory)
